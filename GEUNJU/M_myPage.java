...
public class M_myPage extends JFrame {
	public M_myPage(Connection conn, String ID) throws SQLException {
		...
		//table data -> 헬스장,트레이너 null이면 회원정보 불러오는데 실패함 (query에 null값이 들어가기 때문)
		
		if(!rset.isBeforeFirst()) { //커서가 첫번째 행 앞에 있는지 확인. false면 실행
			JPanel jpErr = new JPanel();
			jpErr.setLayout(new FlowLayout());
			jpErr.add(new JLabel("헬스장과 트레이너 등록 후 사용가능한 메뉴입니다.")); //회원정보를 불러오는데 실패했습니다.
			btnGroup.add(jpErr);
		}
		else {
			while(rset.next()) {
